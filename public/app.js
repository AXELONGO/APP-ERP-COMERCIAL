...
function navigateTo(section) {
  ...
  const hasOwnCreateControl = ['actividades', 'pagos_gastos', 'correos', 'agent', 'playground', 'chats', 'integraciones'].includes(section);
  const addLabels = {
  ...
  document.getElementById('btnDeleteMode').style.display = ['dashboard', 'agent', 'playground', 'chats', 'integraciones'].includes(section) ? 'none' : 'inline-block';
  if (isDeleteMode) toggleDeleteMode();
...
async function openSavedQuote(id) {
  try {
    ...
    quoteDirty = false;
    quoteDraft = { ...quote, contact_id: quote.contact_id, valid_until: quoteDateOnly(quote.valid_until), contact: quote.contact_id ? quoteContacts.find(item => item.id === quote.contact_id) || { id: quote.contact_id, name: quote.contact_name, phone_e164: quote.phone_e164, email: quote.contact_email } : null, items: quote.items || [], payment_plan: quote.payment_plan || { type: 'single', concepts: [] }, fiscal_data: quote.fiscal_data || { requires_invoice: false }, payment_method: quote.payment_method || { type: 'transfer', details: {} } };
...
function downloadQuotePdf(id) {
  if (!id) return showToast('Guarda la cotización antes de descargar el PDF', true);
  window.open(`/quote/${encodeURIComponent(id)}/pdf`, '_blank');
}
...
function quoteDateOnly(value) { const text = String(value || '').trim(); if (/^\d{4}-\d{2}-\d{2}/.test(text)) return text.slice(0, 10); const date = new Date(value); return Number.isNaN(date.getTime()) ? '' : date.toISOString().slice(0, 10); }
...
async function openQuoteWorkspace(contactId = null) {
  try {
    ...
    quoteDirty = false;
    quoteDraft = { id: null, status: 'unsaved', contact_id: contactId, contact: quoteContacts.find(contact => contact.id === contactId) || null, currency: 'MXN', valid_until: new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10), items: [], discount_type: 'fixed', discount_value: 0, tax_rate: 16, payment_plan: { type: 'single', concepts: [] }, fiscal_data: { requires_invoice: false }, payment_method: { type: 'transfer', details: {} }, send_channel: 'whatsapp', message: '' };
    document.getElementById('cotizacionesListView').classList.add('hidden');
    document.getElementById('quoteWorkspace').classList.remove('hidden');
    renderQuoteWorkspace();
...
function renderQuoteBuilder() {
  ...
  if (!builder.querySelector("[name='quote_send_channel']")) builder.insertAdjacentHTML('beforeend', `<details class="quote-accordion" open><summary><span><i class="ph ph-paper-plane-tilt"></i> Envío</span><small>Canal de entrega</small></summary><div class="quote-accordion-body"><label class="form-field">Enviar por<select class="modal-select" name="quote_send_channel" ${locked ? 'disabled' : ''}><option value="link" ${quoteDraft.send_channel === 'link' ? 'selected' : ''}>Enlace público</option><option value="whatsapp" ${quoteDraft.send_channel === 'whatsapp' ? 'selected' : ''}>PDF por WhatsApp</option></select></label><small class="text-muted">La opción WhatsApp envía la cotización como documento PDF al teléfono del contacto.</small></div></details>`);
  const paymentGrid = [...builder.querySelectorAll('.quote-form-grid')].find(grid => grid.querySelector("[name='quote_payment_type']"));
  const paymentType = paymentGrid?.querySelector("[name='quote_payment_type']");
  if (paymentGrid && paymentType) paymentType.parentElement.insertAdjacentHTML('afterend', quotePaymentMethodFields());
...
function syncQuoteDraft() {
  ...
  const nextPaymentType = value('quote_payment_type') || 'transfer';
  const previousPaymentType = quoteDraft.payment_method?.type || 'transfer';
  quoteDraft.payment_method = { type: nextPaymentType, details: Object.fromEntries([...root.querySelectorAll('[data-payment-detail]')].map(field => [field.dataset.paymentDetail, field.value])) };
  ...
  if (previousPaymentType !== nextPaymentType) { renderQuoteBuilder(); renderQuotePreview(); return; }
  renderQuotePreview();
}
...
function quotePaymentMethodFields() { const type = quoteDraft.payment_method?.type || 'transfer'; const details = quoteDraft.payment_method?.details || {}; if (type === 'transfer') return `<label>Banco<input data-payment-detail="bank" value="${escapeDetailHtml(details.bank || '')}" oninput="syncQuoteDraft()" placeholder="Banco receptor"></label><label>Beneficiario<input data-payment-detail="beneficiary" value="${escapeDetailHtml(details.beneficiary || '')}" oninput="syncQuoteDraft()"></label><label>Número de cuenta<input data-payment-detail="account" value="${escapeDetailHtml(details.account || '')}" oninput="syncQuoteDraft()"></label><label>CLABE<input data-payment-detail="clabe" value="${escapeDetailHtml(details.clabe || '')}" oninput="syncQuoteDraft()"></label>`; if (type === 'payment_link') return `<label class="full-width">Enlace de pago<input data-payment-detail="url" type="url" value="${escapeDetailHtml(details.url || '')}" oninput="syncQuoteDraft()" placeholder="https://"></label><label class="full-width">Texto del botón<input data-payment-detail="button_text" value="${escapeDetailHtml(details.button_text || 'Pagar ahora')}" oninput="syncQuoteDraft()"></label>`; return `<label class="full-width">Instrucciones<input data-payment-detail="instructions" value="${escapeDetailHtml(details.instructions || '')}" oninput="syncQuoteDraft()"></label>`; }
...
function renderContactPanel() {
  const panel = document.getElementById('contactPanel');
  if (!panel || !contactPanelData?.contact) return;
...
async function saveContactPanelNote(event) { event.preventDefault(); const form = event.target; const contactId = contactPanelData?.contact?.id; if (!contactId) { closeModal(); return showToast('La ficha del contacto ya no está disponible. Ábrela nuevamente.', true); } const body = form.body.value.trim(); const file = form.attachment.files[0]; if (file && file.size > 2 * 1024 * 1024) return showToast('El archivo no puede superar 2MB', true); const save = attachments => v2Fetch(`/api/v2/contacts/${contactId}/notes`, { method: 'POST', body: JSON.stringify({ body, attachments }) }); try { let attachments = []; if (file) attachments = [{ name: file.name, type: file.type, size: file.size, data: await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.onerror = reject; reader.readAsDataURL(file); }) }]; const result = await save(attachments); if (contactPanelData?.contact?.id === contactId) { contactPanelData.notes = contactPanelData.notes || []; contactPanelData.notes.unshift(result.data); renderContactPanel(); } closeModal(); showToast('Nota agregada'); } catch (error) { showToast(error.message, true); } }
