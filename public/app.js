...
async function openSavedQuote(id) {
  try {
    ...
    quoteDirty = false;
    quoteDraft = { ...quote, contact_id: quote.contact_id, valid_until: quote.valid_until ? String(quote.valid_until).slice(0, 10) : '', contact: quote.contact_id ? quoteContacts.find(item => item.id === quote.contact_id) || { id: quote.contact_id, name: quote.contact_name, phone_e164: quote.phone_e164, email: quote.contact_email } : null, items: quote.items || [], payment_plan: quote.payment_plan || { type: 'single', concepts: [] }, fiscal_data: quote.fiscal_data || { requires_invoice: false }, payment_method: quote.payment_method || { type: 'transfer', details: {} } };
...
async function saveQuoteDraft() { try { const body = quoteRequestBody(); const result = await v2Fetch(quoteDraft.id ? `/api/v2/quotes/${quoteDraft.id}` : '/api/v2/quotes', { method: quoteDraft.id ? 'PUT' : 'POST', body: JSON.stringify(body) }); quoteDraft = { ...quoteDraft, ...result.data, valid_until: result.data.valid_until ? String(result.data.valid_until).slice(0, 10) : quoteDraft.valid_until, items: result.data.items || quoteDraft.items, contact: quoteDraft.contact }; quoteDirty = false; showToast('<i class="ph-fill ph-check-circle" style="color:#10b981"></i> Borrador guardado'); renderQuoteWorkspace(); } catch (error) { showToast(error.message, true); } }
